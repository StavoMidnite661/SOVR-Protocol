...
describe('strict causation regression', () => {
  let store: EventStore;

  beforeEach(() => {
    store = new EventStore(undefined, {
      strictCausation: true,
    });
  });

  it('PASS: valid root event (causation_id === correlation_id)', () => {
  ...
  it('FAIL: random causation_id with no parent', () => {
    store.append({
    ...
    expect(() => store.append({
      event_name: 'test.orphan',
      aggregate: 'test',
      aggregate_id: 'agg-2',
      source_domain: 'test',
      command_id: 'cmd-2',
      triggering_command: 'test.create',
      causation_id: 'random-parent-id',
  ...
  it('PASS: valid child event referencing parent event_id', () => {
    const parent = store.append({
      ...
      actor_id: 'test-actor',
      identity_context: { identity_id: 'test-identity', actor_type: 'human', session_id: 'sess-3' },
  ...
  it('FAIL: child event with missing parent', () => {
    store.append({
      event_name: 'test.seed',
      aggregate: 'test',
      aggregate_id: 'agg-seed',
    ...
    expect(() => store.append({
      ...
      actor_id: 'test-actor',
      identity_context: { identity_id: 'test-identity', actor_type: 'human', session_id: 'sess-4' },
