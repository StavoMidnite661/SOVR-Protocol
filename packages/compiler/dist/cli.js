...
import { stringTreeToBuffers, verifyAgainstTree, verifyTwoIndependentCompiles, } from './determinism.js';
...
async function main() {
    ...
    if (command === 'compile') {
        ...
        if (!proof.identical) {
            ...
            for (const d of proof.differences.slice(0, 40))
                console.error(`  ${d}`);
            if (proof.differences.length > 40)
                console.error(`  ... and ${proof.differences.length - 40} more`);
