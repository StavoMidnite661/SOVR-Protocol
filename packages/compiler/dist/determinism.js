...
export function collectDirTree(dir) {
    const tree = new Map();
...
export function readBuildHash(outDir) {
 ...
 */
export function runIsolatedCompile(cliPath, outDir) {
    const result = spawnSync(process.execPath, [cliPath, '__compile-isolated', '--out', outDir], {
        encoding: 'utf8',
        stdio: ['ignore', 'pipe', 'pipe'],
    });
    if (result.status !== 0) {
        throw new Error(`DETERMINISM_VERIFICATION_FAILED: isolated compilation exited with status ${result.status}\n` +
            `${result.stdout ?? ''}\n${result.stderr ?? ''}`);
    }
}
/**
 ...
 */
export function verifyAgainstTree(cliPath, run1Tree, run1BuildHash) {
 ...
 */
export function verifyTwoIndependentCompiles(cliPath) {
...
export function stringTreeToBuffers(tree) {
    const out = new Map();
