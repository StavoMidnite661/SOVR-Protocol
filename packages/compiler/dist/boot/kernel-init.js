...
function defaultBootSplash(buildHash) {
    return [
...
export async function runBootSequence(rootDir, ir, buildHash) {
    const stages = [];
    const bootLog = [];
    const events = [];
    ...
    // Level 3 Core Domains
    {
        ...
        const vaultEntities = ir.nodes.filter(n => n.domain === 'vault').length;
        const ledgerEntities = ir.nodes.filter(n => n.domain === 'ledger').length;
        const treasuryCommands = ir.nodes.filter(n => n.domain === 'treasury' && n.type === 'command').length;
    ...
    {
        const start = performance.now();
        const identity = ir.nodes.filter(n => n.domain === 'identity').length;
    ...
    const bootSplashSpec = bootSpec?.boot_sequence?.runlevels?.find((rl) => rl.level === 7)?.boot_complete?.boot_splash || defaultBootSplash(buildHash);
    const splash = (Array.isArray(bootSplashSpec) ? bootSplashSpec : [String(bootSplashSpec)]).map((line) => line
 ...
 */
function countProjections(rootDir) {
    const p = join(rootDir, 'projection-engine.yaml');
    if (!existsSync(p))
        return 0;
    const spec = yaml.load(readFileSync(p, 'utf8'));
    const entries = spec?.projections ?? {};
    return Object.values(entries).filter((v) => v && typeof v === 'object' && 'source_domain' in v).length;
}
...
//# sourceMappingURL=kernel-init.js.map