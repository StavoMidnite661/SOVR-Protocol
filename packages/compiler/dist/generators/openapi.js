...
export function generateOpenAPI(ir) {
    ...
    for (const node of ir.nodes) {
        if (node.type === 'command') {
            ...
            const route = `/api/v1/${domain}/${node.aggregate}`;
            ...
            const opId = `${domain}_${node.aggregate}_${cmdName.split('.').pop()}`;
            if (!paths[route])
                paths[route] = {};
            paths[route][method] = {
                operationId: opId,
                summary: cmdName,
                description: `Command ${cmdName} requiring capability ${node.capability}`,
                tags: [domain],
                security: [{ bearerAuth: [] }],
                ...
                'x-constitutional-gates': node.gates,
                'x-capability-required': node.capability,
            };
        }
    }
    const openapi = {
