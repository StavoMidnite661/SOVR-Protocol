...
export function generateCapabilityEngine(ir) {
    ...
    lines.push(`  static readonly ALL_CAPABILITIES = ${JSON.stringify(caps.map(c => c.capabilityId).sort(), null, 2)} as const;`);
    lines.push('}');
    lines.push('');
...
export function generatePolicyEngine(ir) {
    const protocolVersion = ir.meta.protocolVersion;
    const compilerVersion = ir.meta.compilerVersion;
    const lines = [];
