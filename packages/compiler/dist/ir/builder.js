...
export function buildIR(parsed, extraDiagnostics) {
    const nodes = [];
    const edges = [];
    ...
    if (dm?.domains) {
        for (const [domainName, domainDef] of Object.entries(dm.domains)) {
            const ents = domainDef.entities || {};
            for (const [entityName, entityDef] of Object.entries(ents)) {
    ...
    if (sagaCatalog) {
        ...
        for (const saga of sagaLists) {
            ...
            if (saga.domain)
                edges.push({ from: `domain:${saga.domain}`, to: `saga:${sagaId}`, type: 'domain_defines_saga' });
        }
    }
