...
export function generateTLA(ir: SOVR_IR): GeneratedFile[] {
  ...
  for (const sm of stateMachines) {
    ...
    for (const [sName, sDef] of Object.entries(statesMap)) {
      ...
      const transitionsList = (sDef as any).transitions || [];
      for (const t of transitionsList) {
        transitions.push({
          from: sName.toUpperCase(),
          to: (t as any).target.toUpperCase(),
          trigger: (t as any).command ? (t as any).command.replace(/\./g, '_').toUpperCase() : 'TRANSITION'
        });
      }
    }
    ...
    if (states.length === 0) {
      ...
      transitions.push(
        { from: 'INIT', to: 'ACTIVE', trigger: 'ACTIVATE' },
        { from: 'ACTIVE', to: 'COMPLETED', trigger: 'COMPLETE' },
        { from: 'ACTIVE', to: 'FAILED', trigger: 'FAIL' }
      );
    }
