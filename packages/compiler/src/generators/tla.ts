...
export function generateTLA(ir: SOVR_IR): GeneratedFile[] {
  ...
  for (const sm of stateMachines) {
    ...
    const transitionsMap = (sm as any).transitions || {};
    for (const [transitionName, transitionDef] of Object.entries(transitionsMap as any)) {
      ...
      transitions.push({
        ...
        trigger: String((transitionDef as any).trigger ?? (transitionDef as any).command ?? transitionName).replace(/\./g, '_').toUpperCase(),
      });
    }
