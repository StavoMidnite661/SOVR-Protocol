...
async function main() {
  ...
  if (command === 'compile') {
    ...
    const regCount = (name: string): number | string => {
    ...
    const errCount = (code: string) => output.diagnostics.filter(d => d.code === code && (d.severity === 'ERROR' || d.severity === 'FATAL')).length;
    const warnCount = (code: string) => output.diagnostics.filter(d => d.code === code && d.severity === 'WARNING').length;
