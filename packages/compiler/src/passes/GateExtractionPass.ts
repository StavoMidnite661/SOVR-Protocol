...
export class GateExtractionPass {
  ...
  process(commands: YamlCommandSpec[]): {
    ...
    const entries: CompiledCommandEntry[] = []
  ...
  private processCommand(cmd: YamlCommandSpec): CompiledCommandEntry | null {
  ...
  private extractGates(cmd: YamlCommandSpec): GateDefinition[] {
    ...
    for (const yamlGate of cmd.execution_gates) {
      ...
      if (!VALID_GATE_TYPES.has(yamlGate.type)) {
        this.errors.push(
          `Command '${cmd.name}': unknown gate type '${yamlGate.type}'. ` +
      ...
      if (!yamlGate.description) {
        this.warnings.push(
          `Command '${cmd.name}', gate '${yamlGate.gate_id}': missing description`
        )
      }
      ...
      gates.push({
        ...
        type:        yamlGate.type as GateType,
        fatal:       yamlGate.fatal ?? true,
        description: yamlGate.description ?? '',
        config:      yamlGate.config as GateConfig
      })
    }
  ...
  private validateConfig(
    ...
    switch (gateType) {
      ...
      case 'BALANCE_SUFFICIENT':
        if (!config.accountPayloadKey) errors.push(`${ctx}: missing accountPayloadKey`)
        if (!config.amountPayloadKey)  errors.push(`${ctx}: missing amountPayloadKey`)
        if (!config.currency)          errors.push(`${ctx}: missing currency`)
        break
      ...
      case 'AUTHORIZATION_LIMIT':
        if (!config.amountPayloadKey)  errors.push(`${ctx}: missing amountPayloadKey`)
        if (!config.limitSource)       errors.push(`${ctx}: missing limitSource`)
      ...
      case 'TIME_WINDOW':
        if (!config.window) {
          ...
          const w = config.window as any
          if (!w.timezone)                errors.push(`${ctx}: window missing timezone`)
          if (!Array.isArray(w.daysOfWeek)) errors.push(`${ctx}: window missing daysOfWeek array`)
          if (w.startHour === undefined)  errors.push(`${ctx}: window missing startHour`)
          if (w.endHour === undefined)    errors.push(`${ctx}: window missing endHour`)
        }
        break
      ...
      case 'APPROVAL_QUORUM':
        ...
        if (!config.role) errors.push(`${ctx}: missing role`)
        break
      ...
      case 'AMOUNT_WITHIN_LIMIT':
        if (!config.amountPayloadKey) errors.push(`${ctx}: missing amountPayloadKey`)
        if (!config.maxAmount)        errors.push(`${ctx}: missing maxAmount`)
        if (!config.currency)         errors.push(`${ctx}: missing currency`)
        break
    }
